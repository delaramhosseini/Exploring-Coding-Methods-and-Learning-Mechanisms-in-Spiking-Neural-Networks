...
def plot(
    ...
    for i in range(net.iteration):
        similarity.append(
            1
            - cosine(
                sg[recorder_index, 0].variables["W"][i, :, 0].cpu(),
                sg[recorder_index, 0].variables["W"][i, :, 1].cpu(),
            )
        )
    ...
    axd["B"].set_title(
        "PreNeuron Group", fontsize=(label_font_size + 1) * scaling_factor
    )
    axd["B"].set_xlabel(
        f"time ({ngs[0].tag})",
        fontsize=label_font_size * scaling_factor,
    ...
    axd["C"].set_title(
        "PostNeuron Group", fontsize=(label_font_size + 1) * scaling_factor
    )
    axd["C"].set_xlabel(
        f"time ({ngs[1].tag})",
        fontsize=label_font_size * scaling_factor,
    ...
    axd["C"].grid()
    axd["F"].plot(
        sg[recorder_index, 0].variables["W"][:, :, 0].cpu(),
    )
